        ...
        if(Auth::check())
{
            try {
                $validatedData = $request->validate([
                    'content_image' => 'required|image|mimes:jpg,png,jpeg,gif,svg',
                   ]);
               
                $contenttitle=$request->articlecontenttitle;
                $content=$request->articlecontent;
                $contentimagename = $request->file('content_image')->getClientOriginalName();
                $contentimagepath = $request->file('content_image')->store('public/upload');
                $contentimagepath = str_replace('public/','',$contentimagepath);
                $dom = new \DomDocument();
    ...
    public function update($articleid,request $request){
        if($request->has('_token')){
            ...
            $categories = categories::get();
            return view('articleupdate', ['articles'=>$articles,'categories'=>$categories,'selectedCategoriesId'=>$selectedCategoriesId]);
        }
    }
}
